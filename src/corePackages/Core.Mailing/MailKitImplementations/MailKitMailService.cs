...
public class MailKitMailService : IMailService
{
    ...
    {
        if (mail.ToList is null || mail.ToList.Count < 1)
            return;
    ...
    private void EmailPrepare(Mail mail, out MimeMessage email, out SmtpClient smtp)
    {
        ...
        if (mail.CcList is not null && mail.CcList.Any())
            email.Cc.AddRange(mail.CcList);

        if (mail.BccList is not null && mail.BccList.Any())
            email.Bcc.AddRange(mail.BccList);

        email.Subject = mail.Subject;

        if (mail.UnscribeLink is not null)
        ...
        if (_mailSettings.DkimPrivateKey is not null &&
            _mailSettings.DkimSelector is not null &&
