...
public static class IQueryableDynamicFilterExtensions
{
    ...
    public static IQueryable<TEntity> ToDynamic<TEntity>(this IQueryable<TEntity> query, DynamicQuery dynamicQuery)
    {
    ...
    private static IQueryable<TEntity> Sort<TEntity>(IQueryable<TEntity> queryable, IEnumerable<Sort> sort)
    {
        ...
        if (sort.Any())
        {
            string ordering = string.Join(
                separator: ",",
                values: sort.Select(s => $"{s.Field} {s.Dir}"));
    ...
    public static string Transform(Filter filter, IList<Filter> filters)
    {
        ...
        {
            if (filter.Operator.Equals("doesnotcontain"))
